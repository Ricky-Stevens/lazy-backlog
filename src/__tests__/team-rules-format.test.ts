...
describe("evaluateConventions", () => {
  it("returns empty array when no rules provided", () => {
    const result = evaluateConventions(
      { summary: "Add login page", issueType: "Story" },
      [],
    );
  ...
  describe("naming_convention", () => {
    ...
    it('returns "applied" for verb-first summary', () => {
      ...
      expect(result[0].category).toBe("naming_convention");
      expect(result[0].label).toBe("Verb-first summary");
    });

    it('returns "warning" for non-verb-first summary', () => {
      const result = evaluateConventions(
        { summary: "Login page should be added", issueType: "Story" },
        [namingRule],
      );
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("warning");
      expect(result[0].detail).toContain("action verb");
    });
    ...
    it("filters rules by issue_type when set", () => {
      ...
      const result = evaluateConventions(
        { summary: "Fix crash on login", issueType: "Story" },
        [typedRule],
      );
      expect(result).toEqual([]);
    });
  ...
  describe("label_patterns", () => {
    ...
    it('returns "applied" when all labels match kebab-case', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("applied");
      expect(result[0].category).toBe("label_patterns");
    });
    ...
    it("skips when no labels on ticket", () => {
      const result = evaluateConventions(
        { summary: "Add feature", issueType: "Story" },
        [formatRule],
      );
      expect(result).toEqual([]);
    });
  ...
  describe("story_points", () => {
    ...
    it('returns "applied" for valid fibonacci value', () => {
      const result = evaluateConventions(
        { summary: "Add feature", issueType: "Story", storyPoints: 5 },
        [fibRule],
      );
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("applied");
      expect(result[0].detail).toContain("valid fibonacci");
    });

    it('returns "warning" for non-fibonacci value', () => {
      const result = evaluateConventions(
        { summary: "Add feature", issueType: "Story", storyPoints: 4 },
        [fibRule],
      );
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("warning");
      expect(result[0].detail).toContain("not a fibonacci");
    });

    it('returns "info" for median comparison', () => {
      const result = evaluateConventions(
        { summary: "Add feature", issueType: "Story", storyPoints: 8 },
        [medianRule],
      );
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("info");
      expect(result[0].detail).toContain("median 5");
    });
  ...
  describe("component_patterns", () => {
    ...
    it('returns "applied" when all components are known', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("applied");
      expect(result[0].detail).toBe("All components recognized by team");
    });
    ...
    it('returns "warning" for unknown components', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("warning");
      expect(result[0].detail).toContain("Mobile");
    });
  ...
  describe("description_structure", () => {
    ...
    it('returns "warning" when acceptance criteria missing', () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].status).toBe("warning");
      expect(result[0].detail).toContain("Missing acceptance criteria");
    });
  });
});
...
describe("formatConventionsSection", () => {
  ...
  it("formats conventions as markdown table with correct status icons", () => {
    const conventions: AppliedConvention[] = [
      { category: "naming_convention", label: "Verb-first summary", status: "applied", confidence: 0.85, detail: "Matches pattern" },
      { category: "story_points", label: "Fibonacci points", status: "warning", confidence: 0.9, detail: "4 is not fibonacci" },
